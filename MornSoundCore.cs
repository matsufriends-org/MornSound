...
namespace MornSound
{
    public sealed class MornSoundCore
    {
        ...
        public void PlayBgm(MornSoundDataSo info, float volume = 1, float fadeDuration = 1,
            bool skipSameTransition = true, double? scheduled = null)
        {
            if (skipSameTransition && _lastBgmInfo != null && _lastBgmInfo.AudioClip == info.AudioClip) return;

            var soundPlayer = MornSoundPlayer.GetInstance(_solver.transform);
            soundPlayer.Init(_solver.BgmGroup, info.AudioClip, 16, true, volume * info.VolumeRate, info.PitchRate,
                scheduled);
            soundPlayer.FadeIn(fadeDuration);
            if (_cachedBgmPlayer) _cachedBgmPlayer.FadeOut(fadeDuration);
        ...
        private async UniTask FadeInternalAsync(MornSoundVolumeType volumeType, float duration, float endValue,
